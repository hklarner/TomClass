...
def from_NXgraph( Graph, Fname, Parameters, verbose ):
    ...
    for node in Graph.nodes():
        ...
        s+= '   \\node[%s] at (%.1f,%.1f) (%s) {%s};\n'%(ntype,GRAPHICS[node]['x'],GRAPHICS[node]['y'],
                                                         NormId(LABELS[node]),NormLabel(LABELS[node]))
    s += '\n'
    ...
    for source,target in Graph.edges():
        ...
        # OVERRIDE
        if (  LABELS[source],LABELS[target]  ) in OVERRIDE:
            ovr = OVERRIDE[(  LABELS[source],LABELS[target]  )]
            if 'draw' in ovr:
                draw = ovr['draw']
            if 'bend' in ovr:
                bend = ovr['bend']
        ...
        s+= '   \\draw[%s] (%s) %s (%s);\n'%(etype, NormId(LABELS[source]), bend, NormId(LABELS[target]))
